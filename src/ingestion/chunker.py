...
def chunk_python(file_path: str, text: str, max_chunk_size: int = MAX_CHUNK_SIZE) -> List[Chunk]:
    """
    ...
    def add_chunk(start_idx: int, end_idx: int):
        ...
        if len(chunk_text) > max_chunk_size:
            ...
            sub_chunks = chunk_text_by_size(file_path, text, start_idx, end_idx, max_chunk_size)
    ...
    for node in tree.body:
        ...
        if hasattr(node, 'lineno') and hasattr(node, 'col_offset') and hasattr(node, 'end_lineno') and hasattr(node, 'end_col_offset'):
            ...
            try:
                node_end = line_offsets[node.end_lineno - 1] + node.end_col_offset
            except IndexError:
                node_end = len(text)
            ...
            if current_chunk_text and (current_end - current_start + node_length + (node_start - current_end) > max_chunk_size):
...
def chunk_text_by_size(file_path: str, text: str, start_limit: int, end_limit: int, max_chunk_size: int) -> List[Chunk]:
...
def chunk_text(file_path: str, text: str, max_chunk_size: int = MAX_CHUNK_SIZE) -> List[Chunk]:
...
def chunk_markdown(file_path: str, text: str, max_chunk_size: int = MAX_CHUNK_SIZE) -> List[Chunk]:
    """
